...
def execute_sql_scripts(script_directories, log_path):
    ...
    #connection_str = f'SERVER={db_config['server']};DATABASE={db_config['database']};UID={db_config['username']};PWD={db_config['password']};Authentication={db_config['authentication']}'
    connection_str = pyodbc.connect(f"Trusted_Connection= 'No',UID='aa251149@ncr.com',PWD='Iamsourpunk@2023',Authentication='ActiveDirectoryPassword',
              DATABASE='{db_config['database']}',Driver='{db_config['driver']}',Server='atmco-adle-dev-syn-ws01.sql.azuresynapse.net',autocommit=True")
    connection_str.setdecoding(pyodbc.SQL_CHAR, encoding='latin1')
    connection_str.setencoding('latin1')
...
# Example usage
temp_tables_directory = 'Temp_tables'
